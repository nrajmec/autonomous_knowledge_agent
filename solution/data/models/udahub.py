...
class CustomerMemory(Base):
    ...
    summaries, keyed by the internal `user_id` (not a ticket's `thread_id`) so
    they persist across tickets and process restarts. `embedding` is a
    JSON-encoded float vector, populated once at write time and used for
    semantic recall (see `agentic/tools/memory_tools.py`)."""
    ...
    def __repr__(self):
        return f"<CustomerMemory(memory_id='{self.memory_id}', user_id='{self.user_id}', memory_type='{self.memory_type}')>"
